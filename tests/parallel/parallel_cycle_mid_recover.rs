...
pub(crate) fn b3(db: &dyn KnobsDatabase, input: MyInput) -> salsa::Result<i32> {
    // will block on thread a, signaling stage 2
    a1(db, input)
}

fn recover_b3(db: &dyn KnobsDatabase, _cycle: &salsa::Cycle, key: MyInput) -> salsa::Result<i32> {
    dbg!("recover_b3");
    Ok(key.field(db)? * 200 + 2)
}
