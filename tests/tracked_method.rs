...
trait TrackedTrait {
    fn tracked_trait_fn(self, db: &dyn salsa::Database) -> salsa::Result<u32>;
}
...
impl MyInput {
    #[salsa::tracked]
    fn tracked_fn(self, db: &dyn salsa::Database) -> salsa::Result<u32> {
        Ok(self.field(db)? * 2)
    }

    #[salsa::tracked(return_ref)]
    fn tracked_fn_ref(self, db: &dyn salsa::Database) -> salsa::Result<u32> {
        Ok(self.field(db)? * 3)
    }
}
