...
#[salsa::tracked]
fn compute(db: &dyn LogDatabase, input: List) -> salsa::Result<u32> {
...
fn accumulated(db: &dyn LogDatabase, input: List) -> salsa::Result<Vec<u32>> {
    db.push_log(format!("accumulated({:?})", input));
    Ok(compute::accumulated::<Integers>(db, input)?
        .into_iter()
        .map(|a| a.0)
        .collect())
}
