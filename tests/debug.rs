...
#[salsa::tracked]
fn leak_debug_string(_db: &dyn salsa::Database, input: MyInput) -> salsa::Result<String> {
    Ok(format!("{input:?}"))
}
...
#[test]
fn untracked_dependencies() -> salsa::Result<()> {
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        salsa::with_attached_database(|db| {
            write!(
                f,
                "{:?} / {:?}",
                self.my_input(db).map_err(|_| Error)?,
                self.value(db).map_err(|_| Error)?
            )
        })
...
#[test]
fn custom_debug_impl() -> salsa::Result<()> {
