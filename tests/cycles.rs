...
#[salsa::tracked]
fn memoized_a(db: &dyn Db, input: MyInput) -> salsa::Result<()> {
    memoized_b(db, input)
}

#[salsa::tracked]
fn memoized_b(db: &dyn Db, input: MyInput) -> salsa::Result<()> {
    memoized_a(db, input)
}

#[salsa::tracked]
fn volatile_a(db: &dyn Db, input: MyInput) -> salsa::Result<()> {
    db.report_untracked_read();
    volatile_b(db, input)
}

#[salsa::tracked]
fn volatile_b(db: &dyn Db, input: MyInput) -> salsa::Result<()> {
    db.report_untracked_read();
    volatile_a(db, input)
}
...
impl CycleQuery {
    fn invoke(self, db: &dyn Db, abc: ABC) -> salsa::Result<Option<CycleDescription>> {
        match self {
            ...
            CycleQuery::None => Ok(None),
        }
    }
}

#[salsa::tracked(recovery_fn=recover_a)]
fn cycle_a(db: &dyn Db, abc: ABC) -> salsa::Result<Option<CycleDescription>> {
...
fn cycle_b(db: &dyn Db, abc: ABC) -> salsa::Result<Option<CycleDescription>> {
...
#[salsa::tracked]
fn cycle_c(db: &dyn Db, abc: ABC) -> salsa::Result<Option<CycleDescription>> {
    abc.c(db)?.invoke(db, abc)
}

#[track_caller]
fn extract_cycle<F, R>(f: F) -> salsa::Cycle
where
    F: FnOnce() -> salsa::Result<R> + UnwindSafe,
    R: std::fmt::Debug,
{
...
#[test]
fn cycle_memoized() -> salsa::Result<()> {
...
fn expect_cycle() -> salsa::Result<()> {
    ...
    salsa::DatabaseImpl::new().attach(|db| {
        ...
        assert!(cycle_a(db, abc)?.is_some());
        Ok(())
    })
}

#[test]
fn inner_cycle() -> salsa::Result<()> {
    //     A --> B <-- C
    //     ^     |
    //     +-----+
    salsa::DatabaseImpl::new().attach(|db| {
        ...
        "#]];
        expected.assert_debug_eq(&err.unwrap().participants);
...
#[test]
fn cycle_revalidate() -> salsa::Result<()> {
    //     A --> B
    //     ^     |
    //     +-----+
...
#[test]
fn cycle_recovery_unchanged_twice() -> salsa::Result<()> {
    //     A --> B
    //     ^     |
    //     +-----+
    let mut db = salsa::DatabaseImpl::new();
...
#[test]
fn cycle_disappears() -> salsa::Result<()> {
    let mut db = salsa::DatabaseImpl::new();
...
#[test]
fn cycle_disappears_durability() -> salsa::Result<()> {
    let mut db = salsa::DatabaseImpl::new();
    let abc = ABC::new(
        &mut db,
        CycleQuery::None,
    ...
    assert!(cycle_b(&mut db, abc)?.is_none());

    Ok(())
}

#[test]
fn cycle_mixed_1() -> salsa::Result<()> {
    salsa::DatabaseImpl::new().attach(|db| {
        //     A --> B <-- C
        //           |     ^
        //           +-----+
        ...
        "#]];
        expected.assert_debug_eq(&cycle_c(db, abc)?.unwrap().participants);
...
fn cycle_mixed_2() -> salsa::Result<()> {
    salsa::DatabaseImpl::new().attach(|db| {
        ...
        "#]];
        expected.assert_debug_eq(&cycle_a(db, abc)?.unwrap().participants);
        Ok(())
    })
}

#[test]
fn cycle_deterministic_order() -> salsa::Result<()> {
    ...
    "#]];
    expected.assert_debug_eq(&(a.unwrap().participants, b.unwrap().participants));

    Ok(())
}

#[test]
fn cycle_multiple() -> salsa::Result<()> {
    // No matter whether we start from A or B, we get the same set of participants:
    let mut db = salsa::DatabaseImpl::new();
    ...
    expected.assert_debug_eq(&(
        c.unwrap().participants,
        b.unwrap().participants,
        a.unwrap().participants,
    ));
...
#[test]
fn cycle_recovery_set_but_not_participating() -> salsa::Result<()> {
    salsa::DatabaseImpl::new().attach(|db| {
        //     A --> C -+
        //           ^  |
        //           +--+
