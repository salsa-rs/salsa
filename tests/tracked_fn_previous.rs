...
#[salsa::tracked(returns(copy))]
fn depends_on_read_previous_or_value(db: &dyn LogDatabase, input: Switch) -> u32 {
    db.push_log("depends_on_read_previous_or_value".to_owned());
    read_previous_or_value(db, input)
}
...
#[cfg(feature = "accumulator")]
#[salsa::tracked(returns(copy))]
fn accumulates_note(db: &dyn LogDatabase, input: Number) -> u32 {
    db.push_log("accumulates_note".to_owned());
    salsa::Accumulator::accumulate(Note(input.value(db)), db);
    input.value(db)
}

#[cfg(feature = "accumulator")]
#[salsa::tracked(returns(copy))]
fn previous_with_accumulating_dep(db: &dyn LogDatabase, switch: Switch, input: Number) -> u32 {
    db.push_log("previous_with_accumulating_dep".to_owned());

    if switch.use_previous(db) {
        let previous = previous_with_accumulating_dep::previous(db).unwrap();
        return *previous;
    }

    accumulates_note(db, input)
}
...
#[test]
fn previous_value_preserves_early_cutoff_for_dependents() {
    // A query that returns its own previous value keeps a stable output, so backdating must
    // pull its `changed_at` back and spare dependents from re-executing, even though the
    // query itself re-runs because an input it reads changed.
    let mut db = LoggerDatabase::default();
    let input = Switch::new(&db, 1, false);

    assert_eq!(depends_on_read_previous_or_value(&db, input), 1);
    db.clear_logs();
...
#[cfg(feature = "accumulator")]
#[test]
fn previous_replays_accumulated_inputs() {
    let mut db = LoggerDatabase::default();
    let switch = Switch::new(&db, 0, false);
    let input = Number::new(&db, 1);

    assert_eq!(previous_with_accumulating_dep(&db, switch, input), 1);
    let notes = previous_with_accumulating_dep::accumulated::<Note>(&db, switch, input);
    ...
    db.assert_logs(expect![[r#"
        [
            "previous_with_accumulating_dep",
        ]"#]]);
}
