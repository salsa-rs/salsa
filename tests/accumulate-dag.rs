...
#[salsa::tracked]
fn push_logs(db: &dyn Database, input: MyInput) -> salsa::Result<()> {
...
#[salsa::tracked]
fn push_b_logs(db: &dyn Database, input: MyInput) -> salsa::Result<()> {
    // Note that b calls a
    push_a_logs(db, input)?;
...
#[test]
fn accumulate_a_called_twice() -> salsa::Result<()> {
