...
impl<Db: Database> Drop for Handle<Db> {
    fn drop(&mut self) {
...
impl<Db: Database> std::ops::Deref for Handle<Db> {
    type Target = Db;
...
impl<Db: Database> Clone for Handle<Db> {
    fn clone(&self) -> Self {
        *self.coordinate.clones.lock() += 1;
