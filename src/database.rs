...
pub trait Database: Send + ZalsaDatabase {
    ...
    #[doc(hidden)]
    #[inline(always)]
    fn dyn_database(&self) -> &dyn Database {
        // SAFETY: The upcaster is derived from its argument
        unsafe {
            self.zalsa()
                .views()
                .base_database_upcaster()
                .upcast_unchecked(self.into())
        }
    }
    ...
    fn zalsa_register_upcaster(&self) {
        // The no-op upcaster is special cased in view caster construction.
    }
