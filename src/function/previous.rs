    ...
    C: Configuration,
{
    ...
    ///
    /// The active query must be this ingredient's own execution; the returned value's
    /// dependencies are replayed into that query so it inherits the previous result's
    /// durability, edges, untracked reads, changed-at revision, and tracked-struct ids.
    /// Returns `None` if there is no completed memo for the key or it is provisional.
    ///
    /// # Panics
    ///
