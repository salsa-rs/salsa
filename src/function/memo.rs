...
pub struct Memo<'db, C: Configuration> {
    ...
    ///
    /// This must remain the first field: [`ErasedMemo::header`] casts a pointer to the complete
    /// `Memo` allocation to a pointer to `MemoHeader`.
...
///
/// * `data` retains the provenance of a complete `Memo<'_, C>` allocation and is aligned,
///   dereferenceable, and valid for shared access for `'db`.
/// * `to_dyn_fn` and `type_id` describe that same `Memo<'_, C>`. The table represents the
///   lifetime-erased concrete type as `Memo<'static, C>`.
...
impl<'memo> ErasedMemo<'memo> {
    ...
    pub(super) fn downcast<C: Configuration>(self) -> &'memo Memo<'memo, C> {
        ...
        // that the allocation has concrete type `Memo<'static, C>` in the table's lifetime-erased
        // representation. The handle's lifetime invariant permits shortening that erased lifetime
        // to `'memo`, matching the lifetime used by typed memo-table loads.
