    ...
    C: Configuration,
{
    ...
    pub(super) fn execute<'db>(
        ...
        let (new_value, mut revisions) = if C::CYCLE_STRATEGY == CycleRecoveryStrategy::Fixpoint {
            let zalsa_local = active_query.zalsa_local();
            ...
            loop {
                ...
                if revisions.cycle_heads.contains(&database_key_index) {
                    let last_provisional_value = if let Some(last_provisional) =
                    ...
                    if !C::values_equal(&new_value, last_provisional_value) {
                        ...
                        match C::recover_from_cycle(
                            ...
                            crate::CycleRecoveryAction::Fallback(fallback_value) => {
                                tracing::debug!(
                                    "{database_key_index:?}: execute: \
                                    user cycle_fn says to fall back"
                                );
