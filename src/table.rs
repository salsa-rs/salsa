...
struct SlotVTable {
    ...
    drop_impl: unsafe fn(data: *mut (), allocated: usize),
}
...
impl Table {
    ...
    pub(crate) fn slots_of<T: Slot>(&self) -> impl Iterator<Item = &T> + use<'_, T> {
        self.pages
            .iter()
...
impl<'p, T: Slot> PageView<'p, T> {
        ...
        V: FnOnce(Id) -> T,
    {
        ...
        let data = self.0.data.cast::<PageData<T>>();
        // SAFETY: We acquired the allocation lock, so we have unique access to the UnsafeCell interior.
        unsafe { (*data.as_ref()[index].get()).write(value(id)) };
